...
printf '<cmd1>\n<cmd2>\n...\n' | python3 .claude/skills/mud-player/scripts/mud_client.py \
  ...
  [--idle-timeout 1.5] [--max-wait 10] [--raw]
```

- Defaults to `localhost:4000` with account `dummy`/`helloworld` (this
  project's MUD instance) — only pass `--host`/`--port`/`--user`/`--pass`
  to target a different server or account.
