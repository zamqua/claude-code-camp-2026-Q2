...
2. **Act in small batches, not one giant script.** Send a handful of
   ...
   ```
   printf 'look\nnorth\nlook\n' | python3 ${CLAUDE_SKILL_DIR}/scripts/mud_client.py --user <name> --pass <password>
   ```

   This skill is shared across every character/agent playing this MUD, so
   it never hardcodes an account — `--user`/`--pass` are required. Look up
   this character's credentials in the calling project's own `CLAUDE.md`
   (under "Mud Connection") before the first invocation each session. The
   script defaults to `localhost:4000`; pass `--host`/`--port` only if you
   need to point at a different server. `${CLAUDE_SKILL_DIR}` always
   resolves to this skill's own directory regardless of which agent's
   working directory the command runs from — don't hardcode a relative
   path instead. Each line piped to stdin is one game command, sent in
   order, waiting for the game's prompt between each. The output is
   labeled per command so you can tell which response belongs to which
   action.
